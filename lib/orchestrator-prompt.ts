...
- Generate deployment code — call \`updateDeploymentPlatform\` for the user's target (html/typescript/python/react). This auto-generates a complete base file; you do NOT need to write the HTML from scratch.
- Use \`updateDeploymentCode\` ONLY for small, targeted additions on top of the auto-generated base (e.g. a custom CSS block or a short JS snippet). NEVER regenerate a full HTML/TS/PY file in a single tool call — large outputs time out the model and freeze the build.
