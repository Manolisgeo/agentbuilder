...
// Whitelist of ElevenLabs model IDs. The builder spec can carry an invalid
// value (the LLM sometimes hallucinates names like "eleven_labs"), so we
// normalize here before calling the API — invalid/unknown values fall back to
// the safe default.
const VALID_STT_MODELS = ["scribe_v1", "scribe_v1_experimental", "scribe_v2"];
...
function normalizeSttModel(input) {
  return typeof input === "string" && VALID_STT_MODELS.includes(input.trim())
    ? input.trim()
    : "scribe_v2";
}
function normalizeTtsModel(input) {
  return typeof input === "string" && VALID_TTS_MODELS.includes(input.trim())
    ? input.trim()
    : "eleven_turbo_v2_5";
}
...
const server = createServer(async (req, res) => {
      ...
      const resp = await fetch(
        "https://api.elevenlabs.io/v1/text-to-speech/" + voiceCfg.voiceId,
        {
          ...
          body: JSON.stringify({
            ...
            model_id: normalizeTtsModel(voiceCfg.ttsModel),
          }),
        }
      );
      if (!resp.ok) {
  ...
  if (req.method === "POST" && req.url === "/api/stt") {
    ...
    try {
      ...
      form.append("model_id", normalizeSttModel(voiceCfg.sttModel));
